...
export const Users = () => {
  ...
  const handleSubmit = async () => {
    ...
    await axios.post(`/api/users/admin/update/${userId}`, data, {
      headers: {
        'authorization': 'Bearer ' + token
      }
    }).then(res => {
  ...
  const submitPasswordHandler = async (e) => {
    ...
    if (password.newPassword !== password.confirmNewPassword) {
      ...
      await axios.post(`/api/users/admin/change/password/${userId}`, { newPassword: password.newPassword, confirmNewPassword: password.confirmNewPassword }, {
        headers: {
          'authorization': 'Bearer ' + token
        }
      }).then(res => {
    ...
    <div>
      <div>
        ...
        >
          ...
          <Row>
            <Col span={12}>
              <DescriptionItem title="Full Name" content={<span>{user.name}</span>} />
            </Col>
            <Col span={12}>
              ...
              <Form className='editUserForm mt-5'>
                ...
                <Row>
                  <Col span={12} style={{ paddingLeft: '23px' }}>
                    <h6>Name:</h6>
                    <Input style={{ marginBottom: '32px' }} name='name' onChange={handleEditUserChange} value={user.name} />
                  </Col>
                  <Col span={12} style={{ paddingLeft: '23px' }}>
                    <h6>Email:</h6>
